...
function getErrorKey(status: number, errorMsg: string): TranslationKey | null {
  ...
  if (msg.includes("already exists") || msg.includes("already taken") || msg.includes("existe déjà")) {
    return "register_username_exists";
  }
  if (msg.includes("invalid") || msg.includes("invalid data")) {
    ...
    fetch(buildApiUrl("/auth/signup"), {
      ...
      .catch((err) => {
        try {
          ...
          setErrorKey("register_generic_error");
          setErrorMessage("");
        }
      })
      .finally(() => {
    ...
    <div className="max-w-md mx-auto p-6 bg-white dark:bg-[#1E1E2E] rounded-lg shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          {isLoading ? t.register_connecting : t.register_submit}
        </button>
      </form>
