...
export default function UserControlPanel({ username: initialUsername, onStatusChange, mobileTab }: Props) {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setUploadError(t.user_avatar_select_image);
      return;
    }

    if (file.size > 5 * 1024 * 1024) { // 5MB max
      setUploadError(t.user_avatar_size_error);
      return;
    }
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({ error: t.user_avatar_upload_error }));
        setUploadError(error.error || t.user_avatar_upload_error);
        return;
      }
      ...
      setUploadError(t.user_avatar_connection_error);
    } finally {
      setUploading(false);
    }
  };
      ...
      {uploadOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => !uploading && resetUpload()}>
          <div className="bg-[#001839] border border-[#3D3D3D] rounded-xl p-6 w-96 shadow-lg" onClick={e => e.stopPropagation()}>
            <h3 className="text-white font-bold text-lg mb-4">{t.user_avatar_upload_title}</h3>
            ...
            {/* Upload status */}
            {uploading && <p className="text-blue-400 text-sm mb-3">{t.user_avatar_uploading}</p>}
            ...
            <div className="flex gap-2">
              ...
              {selectedFile && (
                ...
                >
                  {uploading ? t.user_avatar_uploading : t.user_avatar_send}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="hidden md:flex fixed z-50 bottom-2 left-3 bg-dark-navy border border-grey-light rounded-xl px-3 py-2 items-center space-x-2 shadow-[0_4px_8px_rgba(0,0,0,0.4)]">
        <div className="flex items-center space-x-2">
          <button
            ...
            title={t.user_avatar_upload_title}
          >
            <Image src={avatar} alt="User" width={48} height={48} className="rounded-full object-cover" />
        ...
        <div className="md:hidden fixed top-16 bottom-16 inset-x-0 z-10 bg-[#001839] overflow-y-auto flex flex-col items-center px-6 pt-10 pb-6 gap-6">
          ...
          <div className="flex flex-col items-center gap-3">
            <button
              ...
              title={t.user_avatar_upload_title}
            >
              <Image src={avatar} alt="User" width={88} height={88} className="rounded-full object-cover border-4 border-blue-500 shadow-lg" />
              <div className="absolute inset-0 rounded-full bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center">
          ...
          <div className="w-full max-w-sm space-y-2">
            <p className="text-gray-400 text-xs uppercase font-bold tracking-wider mb-3">{t.user_status_label}</p>
            {manualOptions.map((opt) => (
              <button
                key={opt.key}
                onClick={() => {
